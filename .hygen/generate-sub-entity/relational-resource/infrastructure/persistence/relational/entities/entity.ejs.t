...
import { <%= h.inflection.classify(enumName) %>Enum } from '@src/<%= h.inflection.transform(parent, ['pluralize', 'underscore', 'dasherize']) %>/enums/<%= h.inflection.transform(enumName, ['underscore', 'dasherize']) %>.enum';
<% }); %>
<% } %>
